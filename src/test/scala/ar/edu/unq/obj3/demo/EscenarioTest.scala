 ...
 */
  ...
  "Escenario Swap en A" should "moverse los personajes de A a distintas celdas" in{

    jugadorA.setearPolitica(0,new Colaborador,new Atacar)
    jugadorA.setearPolitica(1,new Precavido, new EvitarProblemas)

    val enemigoPotenciaAtMenorDefMayor = new Personaje(jugadorB,tablero.celda(1,0))
    enemigoPotenciaAtMenorDefMayor.escudo = 1000
    enemigoPotenciaAtMenorDefMayor.energia = 30
    enemigoPotenciaAtMenorDefMayor.experiencia = 15

    val enemigoPotenciaAtMayorDefMenor = new Personaje(jugadorB,tablero.celda(0,1))
    enemigoPotenciaAtMayorDefMenor.escudo = 0
    enemigoPotenciaAtMayorDefMenor.energia = 90
    enemigoPotenciaAtMayorDefMenor.experiencia = 15

    tablero.celda(1,0).agregarPersonaje(enemigoPotenciaAtMenorDefMayor,jugadorB)
    tablero.celda(0,1).agregarPersonaje(enemigoPotenciaAtMayorDefMenor,jugadorB)

    assert(jugadorA.personajes.apply(0).sugerir == (0,1))
    assert(jugadorA.personajes.apply(1).sugerir == (1,0))

  }

  "Escenario Swap B" should "quedarse los personajes de B en donde estan" in {

    jugadorB.setearPolitica(0, new Pancho, new Atacar)
    jugadorB.setearPolitica(1, new Pancho, new EvitarProblemas)

    assert(jugadorB.personajes.apply(0).sugerir == (3,3))
    assert(jugadorB.personajes.apply(1).sugerir ==(3,3))
